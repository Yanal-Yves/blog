...
**Le pilote propriétaire NVIDIA (`nvidia`) fait exception.** Il n'utilise pas le sous-système DRM/KMS standard et implémente sa propre pile dans le noyau, en dehors des conventions Linux. C'est un choix historique de NVIDIA qui est à l'origine de nombreuses incompatibilités que nous verrons dans cet article (Wayland, uAPI...). Le pilote `nvidia-open`, bien que son code source soit ouvert, reproduit cette même architecture non standard.
...
|---|---|---|---|---|
| **Équipe A (Cartes Récentes)** | `nvidia` (fermé) ou `nvidia-open` (ouvert, via puce GSP)<br/>*Non standard — pile propriétaire hors DRM/KMS* | Pilote fermé NVIDIA :<br/>**Vulkan** : `libnvidia-vulkan-producer.so`<br/>**OpenGL** : `libGLX_nvidia.so` / `libnvidia-glcore.so`<br/>**CUDA** : `libcuda.so` / `libcudnn.so` | Turing et supérieur<br/>(Séries GTX 16xx, RTX 20, 30, 40, 50+)<br/>*Pilotes modernes (ex: 555+)* | **X11** : Excellent.<br/>**Wayland** : Très bon (support natif GBM).<br/>**CUDA/IA** : Excellent et indispensable.<br/>**Jeux 3D** : Excellent. |
| **Équipe A (Cartes Anciennes)** | `nvidia` (fermé, pilotes "Legacy")<br/>*Non standard — pile propriétaire hors DRM/KMS* | Pilote fermé NVIDIA<br/>(Mêmes .so, mais anciennes versions) | Kepler, Maxwell, Pascal<br/>(Séries GTX 600, 700, 900, 1000)<br/>*Pilotes Legacy (ex: 470.xx)* | **X11** : Bon (stable historiquement).<br/>**Wayland** : Inutilisable (pas de support GBM).<br/>**CUDA/IA** : Limité aux vieux logiciels.<br/>**Jeux 3D** : Bon (performances correctes sous X11). |
